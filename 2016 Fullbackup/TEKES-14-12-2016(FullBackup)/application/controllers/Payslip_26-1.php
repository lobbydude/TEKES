            ...
            while (($payslipData = fgetcsv($file, 10000, ",")) !== FALSE) {
                if ($n != 1) {
                    ...
                    if ($count_payslip == 0) {
                        ...
                        $night_shift = $payslipData[14];
                        $weekend = $payslipData[15];
                        $referal_bonus = $payslipData[16];
                        $additional_others = $payslipData[17];
                        $incentives = $payslipData[18];
                        $Basic = ($Monthly_CTC * 45) / 100;
                        if ($Basic >= 8000) {
                            $Basicpay = $Basic;
                        } else {
    ...
    function export_payslip() {
        ...
        if ($count_payslip != 0) {
            foreach ($q_payslip->result() as $row_payslip) {
                ...
                $Per_Day_Salary = $Total_Fixed_Gross_Company / 30;
